...
So far, we've deployed and configred Skupper and the application manually using `skupper` and `kubectl`.
...
To install Skupper and Kubernetes modules you need to use  the `ansible-galaxy` command already installed with Ansible.
...
With modules installed, we can move forward to store the Kubernetes `kubeconfig` files of noth clusters in the local directory:

=== Kubeconfigs

To store the `kubeconfig` of the *cluster 1* export `KUBECONFIG` environment variable to the following value and then run `oc login` to craete the file with the required values:
...
Now, we need to store the information of the *cluster 2* into another `kubeconfig` file.
First unset the previous `KUBECONFIG`, switch to _kind_ context, and finally use `kind` tool to materialize the file:
...
At this point, we can refer to these files in Ansible to login to the clusters.
...
First of all, we need to create the Kubernetes manifests to deploy the application.
They are the same used before, but as a reminder we'll paste it here again.
Store them into the `kubernetes` folder, so you can refer them from Ansible playbook:
...
Now, we need to create the two most important files in Ansible, the inventory and the playbook file.
In this case, we are going to call the *cluster 1* west, and *cluster 2* east, as these names makes the playbook look more clear.
...
<2> Sets the `kubeconfig` file created in previous section
<3> Sets the namespace
<4> Defines the east cluster
<5> Defines the link between clusters
<6> Exposes the postgres service to west
...
<4> Apply the role for installing Skupper
