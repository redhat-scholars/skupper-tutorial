...
Let's see the first use case of hybrid cloud.
Suppose you've got a backend application that connects to a database to find some information.

This database should be in a different cluster (or region) for any reason, maybe data locality, or on-prem cluster, or legacy system running in an old datacenter.

In this example, we'll deploy the backend in the *cluster 1* and database in the *cluster 2*, and connect them as they were in the same cluster:
...
The first thing you ned to do is installing Skupper proxy in both clusters.

In *cluster 1*, we'll install apart from the proxy, the Skupper data collector and the UI, while only Skupper proxy in *cluster 2*.

=== Cluster 1

Let's switch to *cluster 1* context and install Skupper using the `skupper` CLI.
...
After installation, you have the infrastructure you need, but your namespaces are not linked.
There is no network tunnel between both namespaces. So let's create this connection.

First, we need to switch to *cluster 1* and create a token with all information regarding this cluster.
...
You can check the status of the link by calling `skupper link status`, as it might taken some seconds till the connecxtion is established:
...
Let's start deploying the PoostgreSQL database into *cluster 2* (KinD).
...
----
<1> For deep dive proposes we stick to `ConfigMap`. For production you should use a secret management.
...
After some seconds, postgreSQL database is running in our local cluster.
...
Let's switch the context to *cluster 1*, remember that we have not deployed anything apart from Skupper in that cluster yet, and inspect the Kubernetes services presents:
...
----
<1> A new Kubernetes Service poiting to the other cluster.

You see that even though we created the PostgreSQL instance in *cluster 2*, this *cluster 1* also has a Kubernetes service named `postgres` (the same name as in *cluster 2*).
Every time a request is sent to `postgres` service in Kubernetes *cluster 1* is automatically redirected to `postgres` service in Kubernetes *cluster 2* using the tunnel linked in the previous section. 

=== Backend

Let's deploy the backend to *cluster 1*. 
This application will use `postgres` hostname as database hostname as it was deployed in the local cluster, and from its point of view , it will seem so, but thanks of Skupper the requests will be sent to the other cluster.
...
To test the application let's send a request to backend requesting a list of all persons stored in the database.
Notice that the flow will be from your laptop you'll send an HTTP request to *cluster 1*, then a query will be sent to database deployed in *cluster 2*, the result will come back to *cluster 1*, which finally are back to your laptop. 
Long journey, super simple to configure it thanks of Skupper.
...
You can access to Skupper UI to get an overview of the clusters, or consult collected that like traffic send through clusters.
